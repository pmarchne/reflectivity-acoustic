...
def numpy_reflectivity_p_adj(
    ...
    dR_dh = np.zeros((nw, nk, nlay), dtype=np.complex128)
    ...
    for iw, omega in enumerate(omegas):
        ...
        for ell in range(nlay - 2, -1, -1):
            ...
            D = 1.0 + r * Rstep[ell + 1] * np.exp(2j * kz[ell + 1] * h[ell + 1])
            Rstep[ell] = (r + Rstep[ell + 1] * np.exp(2j * kz[ell + 1] * h[ell + 1])) / D

        phi = np.exp(2j * kz * h[:, None])

        if free_surface:
            cavity = 1.0 / (1.0 + Rstep[0] * phi[0])
        ...
        adj_h = np.zeros((nlay, nk), dtype=np.complex128)
        ...
        if free_surface:
            ...
            adj_Rstep0 += adj_t * phi[0]
            adj_kz[0] += adj_t * Rstep[0] * (2.0j * h[0] * phi[0])
            adj_h[0] += adj_t * Rstep[0] * (2.0j * kz[0] * phi[0])
        ...
        for ell in range(nlay - 1):
            ...
            adj_h[ell + 1] += adj_q * (2.0j * kz[ell + 1] * q)
        ...
        for ell in range(nlay):
            ...
            dR_dh[iw, :, ell] = adj_h[ell]

    return R, dR_dvp, dR_drho, dR_dh


def reflectivity_p_adj(layers, omegas, p, **kwargs):
    omegas = np.asarray(omegas, dtype=np.complex128)
    p = np.asarray(p, dtype=np.float64)
    R, dR_dvp, dR_drho, dR_dh = numpy_reflectivity_p_adj(layers, omegas, p, **kwargs)
    return R, dR_dvp, dR_drho, dR_dh
...
def gradient_check(layers, omegas, p, eps=1e-8, print_info=False, **kwargs):
    ...
    R, dR_dvp, dR_drho, dR_dh = numpy_reflectivity_p_adj(layers, omegas, p, **kwargs)
    max_err_vp, max_err_rho, max_err_h = np.zeros_like(vp0), np.zeros_like(rho0), np.zeros_like(vp0)
    for ell in range(len(vp0)):
        vp_cs = vp0.astype(np.complex128)
        vp_cs[ell] += 1j * eps
        layers_cs = np.column_stack([h, vp_cs, rho0])
        ...
        rho_cs[ell] += 1j * eps
        layers_cs = np.column_stack([h, vp0, rho_cs])
        R_cs = numpy_reflectivity_p(layers_cs, omegas, p, **kwargs)
        cs_rho = (R_cs - R) / (1j * eps)
        max_err_rho[ell] = np.max(np.abs(cs_rho - dR_drho[:, :, ell]))

        h_cs = h.copy().astype(np.complex128)
        h_cs[ell] += 1j * eps
        layers_cs = np.column_stack([h_cs, vp0, rho0])
        R_cs = numpy_reflectivity_p(layers_cs, omegas, p, **kwargs)
        cs_h = (R_cs - R) / (1j * eps)
        max_err_h[ell] = np.max(np.abs(cs_h - dR_dh[:, :, ell]))
        ...
        if print_info:
            print(
                ...
                f"h max absolute err = {max_err_h[ell]:.3e}"
            )
    return max_err_vp, max_err_rho, max_err_h
...
def fortran_reflectivity_adj(layers, omegas, p, free_surface=1, zr=70.0, zs=80.0):
    ...
    R, dR_dvp, dR_drho, dR_dh = rfadjmod.compute_reflectivity_adj(
...
def benchmark_adj():
    ...
    for _ in range(repeats):
        with timer("numpy reflectivity adjoint"):
            R, dR_dvp, dR_drho, dR_dh = numpy_reflectivity_p_adj(
                layers, omegas, p, free_surface=fs, zr=zr, zs=zs
            )
        if FORTRAN_AVAILABLE:
            with timer("fortran reflectivity adjoint"):
                Rf, dRf_dvp, dRf_drho, dRf_dh = fortran_reflectivity_adj(
                    layers, omegas, p, free_surface=fs, zr=zr, zs=zs
                )
        ...
        err_fortran_dh = np.max(np.abs(dRf_dh - dR_dh))
