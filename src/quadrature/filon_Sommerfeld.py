...
from src.quadrature.filon import compute_prop_direct, build_filon_rule, transform_reflectivity
...
def Sommerfeld_integral2D(
    ...
    # transform once, outside the receiver loop
    with timer("quadrature"):
        R_trans = transform_reflectivity(R_prop, Ntheta-1, len(rule["nodes"]), rule["T"], rule["inverse"])
        acc_prop = compute_prop_direct(dz_vec, dx_vec, k0_vec, thetas, R_trans)
