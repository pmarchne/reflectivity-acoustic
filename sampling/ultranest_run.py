...
def run_ultranest_inference(bayes, param, v_true, save_dir='dir'):
    ...
    result = sampler.run(min_num_live_points=400, dlogz=0.5, min_ess=1000, update_interval_volume_fraction=0.4, max_num_improvement_loops=5)  # 1000
...
if __name__ == "__main__":
    ...
    if case == 'synthetic':
        ...
        m_ref = np.array([1800.0, 3500.0, 150.]) # reference model used to generate the observations
        ...
        bayes_model, param = prepare_fd_model(file_path=path, seed=seed, debug=True)
    ...
    # Launch UltraNest
    save_dir = 'ultranest_results_' + case
